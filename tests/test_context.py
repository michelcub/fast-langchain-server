...
class TestAgentContextFactory:
    ...
    def test_from_request_two_calls_produce_different_ids(self):
        ctx1 = AgentContext.from_request(session_id="s", user_input="hi", headers={})
        ctx2 = AgentContext.from_request(session_id="s", user_input="hi", headers={})
        assert ctx1.request_id != ctx2.request_id

    def test_from_request_defaults(self):
        ctx = AgentContext.from_request(session_id="s", user_input="hi", headers={})
    ...
    def test_from_request_custom_model(self):
        ctx = AgentContext.from_request(
            session_id="s", user_input="hi", headers={}, model="gpt-4o"
        )
